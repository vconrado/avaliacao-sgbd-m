...
echo "Produzindo doys ..."
cat $DIR/hdf.files | cut -d '.' -f 2 | sort | uniq > $DIR/hdf.files.doys
# numerar doys
COUNT=0
while read line; do
    echo $line,$COUNT >>  $DIR/hdf.files.doys.numered
    COUNT=$(( COUNT + 1 ))
done < $DIR/hdf.files.doys
...
while read line; do
    ...
    DAYSEQ=$(grep "$AYEARDOY" "$DIR/hdf.files.doys.numered" | cut -d ',' -f 2)
    #     1    ,             2     ,     3      ,     4    ,    5       ,    6     ,   7  ,  8      
echo "${DAYSEQ},${FILE_DIR}/${FILE},${COL_START},${COL_END},${ROW_START},${ROW_END},${DOY},${YEAR}" >> $DIR/hdf.files.csv
done < $DIR/hdf.files

echo "Ordenando linhas do csv ..."
sort -t ',' -k 1 -n $DIR/hdf.files.csv > $DIR/hdf.files_sorted.csv
